...
public class Parser {
    ...
    public Factor parseFactor() {
        ...
        if (lexer.now().equals("(")) {     //表达式因子
            ...
            return parseDerivatives();
        } else {        //custom function
            return parseCustomFunc();
        }
    }

    public Derivatives parseDerivatives() {
        lexer.next();
        Derivatives derivatives = new Derivatives();
        derivatives.setVar(lexer.now());
        lexer.next();
        Factor exprFactor = parseFactor();
        derivatives.setPolyFactor(exprFactor.toPoly());
        return derivatives;
    }
