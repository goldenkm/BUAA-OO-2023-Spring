...
public class Mono {
    ...
    public Mono clone() {
        Mono clonedMono = new Mono();
        clonedMono.setNum(this.num);
        clonedMono.setIndexX(this.indexX);
        clonedMono.setIndexY(this.indexY);
        clonedMono.setIndexZ(this.indexZ);
        HashMap<Poly, Integer> clonedSin = new HashMap<>();
        for (Map.Entry<Poly, Integer> entry : this.sin.entrySet()) {
            Poly clonedPoly = entry.getKey().clone();
            clonedSin.put(clonedPoly, entry.getValue());
        }
        clonedMono.setSin(clonedSin);
        HashMap<Poly, Integer> clonedCos = new HashMap<>();
        for (Map.Entry<Poly, Integer> entry : this.cos.entrySet()) {
            Poly clonedPoly = entry.getKey().clone();
            clonedCos.put(clonedPoly, entry.getValue());
        }
        clonedMono.setCos(clonedCos);
        return clonedMono;
    }
}
